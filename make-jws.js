...
async function makeJws(srv_input) {
    ...
    console.log(JSON.stringify({
        payload: signedBody.payload, 
        signature: signedBody.signature,
        protected: new Buffer.from(JSON.stringify(header)).toString('base64').replace(/=+$/, "") }))

}

var stdinBuffer = fs.readFileSync(0); // STDIN_FILENO = 0
makeJws(JSON.parse(stdinBuffer.toString()));
